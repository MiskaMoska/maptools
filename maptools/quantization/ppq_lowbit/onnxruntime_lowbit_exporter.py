...
class ONNXRUNTIMLowBitExporter(OnnxExporter):
    """
    ...
    #     offset_dtype = f"int{config.num_of_bits}"
    #     value_dtype = f"int{config.num_of_bits}"
    #     if config.policy.has_property(QuantizationProperty.ASYMMETRICAL):
    #         offset_dtype = f"uint{config.num_of_bits}"
    #         value_dtype  = f"uint{config.num_of_bits}"
    ...
    def insert_quantize_node(
        ...
        if config.policy.has_property(QuantizationProperty.LINEAR):
            ...
            scale = convert_any_to_torch_tensor(config.scale.clone(),
                                                dtype=torch.float32)
            # offset = ppq_tensor_round(config.offset.clone()).type(
            # offset_dtype)
            offset = ppq_tensor_round(config.offset.clone())

            created = graph.create_operation(op_type='QuantizeLinear',
                                             attributes={})
            ...
            if var in op.inputs:
                graph.insert_op_before(A=created, B=op,
                                       input_idx=op.inputs.index(var))
            elif var in op.outputs:
                graph.insert_op_after(A=created, B=op,
                                      output_idx=op.outputs.index(var))
            else:
                raise ValueError(
                    f'Unexpected Error in Exporting Op {op.name}({op.type}).')

            graph.create_variable(name=None, value=scale, is_parameter=True,
                                  dest_ops=[created])
            graph.create_variable(name=None, value=offset, is_parameter=True,
                                  dest_ops=[created])
            ...
            scale = convert_any_to_torch_tensor(config.scale.clone(),
                                                dtype=torch.float32)
            offset = convert_any_to_torch_tensor(config.offset.clone(),
                                                 dtype=torch.float32)
    ...
    def insert_dequantize_node(
        ...
        if config.policy.has_property(QuantizationProperty.LINEAR):
            ...
            # offset_dtype)
            offset = ppq_tensor_round(config.offset.clone())
            created = graph.create_operation(op_type='DequantizeLinear',
                                             attributes={})
            if config.policy.has_property(QuantizationProperty.PER_CHANNEL):
                created.attributes['axis'] = config.channel_axis
            else:
    ...
    def remove_activation_ops(self, graph: BaseGraph) -> BaseGraph:
        ...
        for op in removed_activations:
            ...
            if input_var.source_op is not None and input_var.source_op.type in {
                'DequantizeLinear', 'DequantizeFloating'}:
                ...
                zeropoint_diff = torch.max(
                    torch.abs(offset - config.offset)).item()
                if scale_diff < 1e-4 and zeropoint_diff < 1e-1:
                    continue

            if len(output_var.dest_ops) == 1 and output_var.dest_ops[
                0].type in {'QuantizeLinear', 'QuantizeFloating'}:
                assert output_var.dest_ops[
    ...
    def remove_duplicated_quant_op(self, graph: BaseGraph) -> BaseGraph:
        ...
        for qt_op in graph.operations.values():
            if qt_op.type == 'QuantizeLinear':
                ...
                if graph.get_upstream_operations(qt_op)[
                    0].type != 'DequantizeLinear': continue
                interested_pairs.append(
                    (qt_op, graph.get_upstream_operations(qt_op)[0]))

        mark_to_remove = set()
        for qt_op, dq_op in interested_pairs:
            assert isinstance(dq_op, Operation) and isinstance(qt_op, Operation)
            scale_diff = torch.max(
                torch.abs(dq_op.inputs[1].value - qt_op.inputs[1].value)).item()
            zeropoint_diff = torch.max(
                torch.abs(dq_op.inputs[2].value - qt_op.inputs[2].value)).item()
            if scale_diff < 1e-5 and zeropoint_diff < 0.5:  # zero point 是整数，所以只要误差小于1就行了。
                ...
                assert len(graph.get_downstream_operations(
                    qt_op)) == 1, 'Oops, that should never happen.'
    ...
    def convert_operation_from_opset11_to_opset13(self,
        ...
        for op in graph.operations.values():
            if op.type == 'ReduceSum' or op.type == 'Squeeze' or op.type == 'Unsqueeze':
                ...
                axes = convert_any_to_torch_tensor(op.attributes.pop('axes'),
                                                   dtype=torch.int64)
                graph.create_variable(name=None, value=axes, is_parameter=True,
                                      dest_ops=[op])

            elif op.type == 'Split':
                if 'split' not in op.attributes: continue  # split is already v13
                split = convert_any_to_torch_tensor(op.attributes.pop('split'),
                                                    dtype=torch.int64)
                graph.create_variable(name=None, value=split, is_parameter=True,
                                      dest_ops=[op])
    ...
    def convert_operation(self, graph: BaseGraph, op: QuantableOperation,
        ...
        for config, var in [_ for _ in op.config_with_variable]:
            ...
            if not QDQHelper.TQC_Exportable_Check(TQC=config,
                                                  bounded_var=var): continue
            ...
            if var.is_parameter:
                ...
                if not quantized_param:
                    created = self.insert_quantize_node(
                        graph=graph, var=inserting_var, config=config,
                        op=inserting)
                ...
                if len(var.dest_ops) == 1 and var.dest_ops[0].type in {
                    'QuantizeLinear', 'QuantizeFloating'}:
                    assert var.dest_ops[
                               0].num_of_input == 3, 'Quantize Node Format Error, need as least 3 inputs.'
    ...
    def prepare_graph(
        ...
        for op in graph.topological_sort():
            ...
            self.convert_operation(graph=graph, op=op,
                                   quantized_param=quant_parameter_to_int)
        ...
        for op in graph.operations.values():
            if isinstance(op, QuantableOperation):
                for cfg, var in op.config_with_variable:
                    if not QDQHelper.TQC_Exportable_Check(TQC=cfg,
                                                          bounded_var=var): continue
                    if cfg.policy.has_property(
